...
public class PostDetailsActivity extends AppCompatActivity implements View.OnClickListener, PostReviewAdapter.ItemListener {
    ...
    private String likePush = "";
    private String likePushParent = "";
...
//                        likeFileBtn.setEnabled(false);
                        likePush = likeList.getKey();
                        Log.e("LOKE", likeList.getKey());
                        likeFileBtn.setBackgroundColor(Color.BLUE);
                        likestatus = true;
    ...
    private void likePost() {
        ...
        PostLikeModel postLikeModel = new PostLikeModel(firebaseAuth.getCurrentUser().getUid());
            ...
            if (likestatus) {
                databaseReference
                        .child(AppConstant.FIREBASE_TABLE_POST_LIKES)
                        .child(postPushKey)
                        .child(likePush)
                        .removeValue(new DatabaseReference.CompletionListener() {
                            @Override
                            public void onComplete(DatabaseError databaseError, DatabaseReference databaseReference) {
                                likeFileBtn.setBackgroundColor(getResources().getColor(R.color.colorPrimary));
                                Log.e("LIKEMORE", databaseReference.getRef() + ":");
                                Log.e("LIKEMORE", likePush + ":");
                                likestatus = false;
        ...
        if (!likestatus) {
            databaseReference
                    ...
                    .child(postPushKey)
                    .push()
                    .setValue(postLikeModel).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
