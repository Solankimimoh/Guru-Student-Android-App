...
public class HomeActivity extends AppCompatActivity
        implements NavigationView.OnNavigationItemSelectedListener, RecyclerViewAdapter.ItemListener, AddPostAdapter.ItemListener, AdapterView.OnItemSelectedListener {
...
//        recyclerView.setLayoutManager(layoutManager);
    ...
    private void initView() {
        ...
        industrySpinner = findViewById(R.id.depart_list);
...
//                break;
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        postFilter = parent.getSelectedItem().toString();
        Toast.makeText(this, "" + postFilter, Toast.LENGTH_SHORT).show();


        if(addPostModelArrayList.size()>0)
        {
            if (postFilter.equals("All")) {
                ArrayList<AddPostModel> addPostModelArrayList1 = new ArrayList<>();
                addPostModelArrayList1.addAll(addPostModelArrayList);
                addPostAdapter.updateList(addPostModelArrayList1);

            } else {
                ArrayList<AddPostModel> addPostModelArrayList1 = new ArrayList<>();
                for (int i = 0; i < addPostModelArrayList.size(); i++) {
                    if (postFilter.equals(addPostModelArrayList.get(i).getIndustry())) {
                        addPostModelArrayList1.add(addPostModelArrayList.get(i));
                    }
                }
                addPostAdapter.updateList(addPostModelArrayList1);
                addPostAdapter.notifyDataSetChanged();
